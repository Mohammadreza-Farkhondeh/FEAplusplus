...
void Preprocessor::generateMesh(const std::vector<std::vector<double>>& nodeCoordinates, 
...
void Preprocessor::applyBoundaryConditions(const std::vector<std::vector<int>>& boundaryNodeIndices,
    ...
    for (size_t i = 0; i < boundaryNodeIndices.size(); ++i) {
        for (size_t j = 0; j < boundaryNodeIndices[i].size(); ++j) {
            ...
            mesh.applyBoundaryCondition(mesh.getNodes()[nodeIndex], boundaryDOFIndices[j], boundaryValues[j]);
        }
    }
}

void Preprocessor::applyLoads(const std::vector<int>& loadNodeIndices,
