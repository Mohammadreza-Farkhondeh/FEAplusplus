...
DistributedLoad::DistributedLoad(double totalLoad, const std::vector<Node*>& nodes, std::function<std::vector<double>(const std::vector<Node*>&)> distributionFunction) 
    : totalLoad(totalLoad), nodes(nodes), distributionFunction(distributionFunction) {
    if (nodes.empty()) {
...
void DistributedLoad::apply(Node& node) const {
    ...
    if (loads.size() != nodes.size()) {
        throw std::runtime_error("The size of loads vector does not match the size of nodes vector.");
    }
